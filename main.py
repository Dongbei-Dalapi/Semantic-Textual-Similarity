...
from lstm_model import BERT_Model, LSTM_Model, MaLSTM, BertSemanticDataGenerator
...
if __name__ == '__main__':
    ...
    if not args.BERT:
        ...
        p = lstm_model.predict(train_seq1, train_seq2)
        print(p)
        predictions = []
        gold_labels = []
        for i in range(len(indexs)):
            if i != len(indexs)-1:
                predictions.append(lstm_model.predict(
                    test_seq1[indexs[i]:indexs[i+1], :], test_seq2[indexs[i]:indexs[i+1], :]))
                gold_labels.append(test_labels[indexs[i]:indexs[i+1]])
        all_p = lstm_model.predict(test_seq1, test_seq2)
